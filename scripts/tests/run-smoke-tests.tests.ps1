...
$installerScenario = $manifestJson.scenarios | Where-Object { $_.name -eq "tropicana-bootstrap-install" } | Select-Object -First 1
Assert-True -Condition ($null -ne $installerScenario) -Message "Smoke manifest should define a Tropicana bootstrap installer scenario"
...
Assert-True -Condition ($installerScenario.kind -eq "bootstrap-install") -Message "Installer scenario should use bootstrap-install kind"
...
Assert-True -Condition ($runnerContent.Contains("bootstrap-install")) -Message "Smoke runner should handle bootstrap installer scenarios"
