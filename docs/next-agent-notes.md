...
- Guided upgrade now prefers a published installer asset from the latest GitHub Release, downloads it into `updates/`, and launches the packaged installer helper. Keep the release workflow publishing the `.exe` asset, or the GUI will fall back to opening the release page.
